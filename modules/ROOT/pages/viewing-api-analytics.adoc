...
[Important]
In the line chart, numbers of requests are not normalized for custom intervals. For example, monthly chart will show number of requests per 4 hours. For two months, the same chart shows requests per 8 hours. Bart chart and ring chart always display correct numbers. For line charts in custom dashboards, aggregation intervals set explicitly.
